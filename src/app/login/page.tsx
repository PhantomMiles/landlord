...
import { loginAction } from "@/app/actions/actions-auth";

// useSearchParams MUST be in a separate component wrapped by Suspense
function ErrorBanner() {
  const searchParams = useSearchParams();
  const error = searchParams.get("error");
  if (!error) return null;
  return (
    <div className="mb-4 px-4 py-2.5 bg-rose-500/10 border border-rose-500/20 rounded-xl">
  ...
  return (
    <div className="z-10 w-full max-w-sm">
      ...
      <Card className="bg-white/5 backdrop-blur-2xl border-white/10 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-500 rounded-[2rem]">
        ...
        <CardContent>
          {/* Error banner is isolated in its own Suspense boundary */}
        ...
        <CardFooter className="pb-8 pt-2">
          <div className="relative w-full">
            ...
            <div className="relative flex justify-center text-[9px] font-bold uppercase tracking-widest">
              <span className="bg-transparent px-3 text-slate-600">Secure Entry</span>
            </div>
          </div>
        </CardFooter>
      </Card>
